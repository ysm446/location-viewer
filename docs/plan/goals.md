...
- **データ単位**: 「ワークスペース」が最上位。ハイトマップ（地形＝更新可能）と地点（ランドマーク）が
  ワークスペースに紐づく。保存は `data/<id>/`（`workspace.json` / `heightmap.u16` / `preview.png` /
  `satellite.png`）。地点は lng/lat 保持なので地形を更新しても残る。
...
- バックアップ: ロケーション単位で ZIP に書き出し／取り込み（このアプリで復元できる丸ごと一式）。
